...
BOOL CMainWnd::AdjustWndPos()
{
	...
	rtBoardPart.left = 0;
	rtBoardPart.top = 0;
	rtBoardPart.right = (int)(rtClient.right*RATIO - 1);
	rtBoardPart.bottom = rtClient.bottom - 1;
	GetBkAndBoardRT();//计算主体界面图像位置

	/*设置信息板显示部分在主窗体客户区的位置*/
	rtInfoPart.left = rtBoardPart.right + 1;
	rtInfoPart.top = 0;
	rtInfoPart.right = rtClient.right - 1;
	rtInfoPart.bottom = rtClient.bottom - 1;
...
VOID CMainWnd::GetBkAndBoardRT()
{
	...
	//计算棋盘边长
	BoardSide = ((rtBoardPart.right - rtBoardPart.left) > (rtBoardPart.bottom - rtBoardPart.top) ? (rtBoardPart.bottom - rtBoardPart.top) : (rtBoardPart.right - rtBoardPart.left)) + 1;
	BoardSide -= (MARGIN * 2);

	//计算棋盘位置
	rtBoard.left = (int)((rtBoardPart.right + rtBoardPart.left + 1 - BoardSide) / 2 - 1);
	rtBoard.right = rtBoard.left + BoardSide - 1;
	rtBoard.top = (int)((rtBoardPart.bottom + rtBoardPart.top + 1 - BoardSide) / 2 - 1);
...
BOOL CMainWnd::AdjustCtrlPos()
{
	...
	//基准偏移
	edge.cx = (int)((rtInfoPart.right - rtInfoPart.left + 1)*0.05);
	edge.cy = (int)((rtInfoPart.bottom - rtInfoPart.top + 1)*0.05 / 3);

	//黑方分组框
	rtSBlc.left = rtInfoPart.left;
	rtSBlc.right = rtInfoPart.right;
	rtSBlc.top = rtInfoPart.top;
	rtSBlc.bottom = rtSBlc.top + (int)(rtInfoPart.bottom / 3) - 1;

	//黑方引擎名静态框
	rtBlcName.left = rtSBlc.left + 2 * edge.cx;
	rtBlcName.right = rtSBlc.right - 2 * edge.cx;
	rtBlcName.top = rtSBlc.top + 2 * edge.cy;
	rtBlcName.bottom = rtBlcName.top + (int)((rtSBlc.bottom - rtSBlc.top + 1 - 7 * edge.cy) / 4) - 1;

	dy = rtBlcName.bottom - rtBlcName.top;

	//黑方计时静态框
	rtBlcTime.left = rtBlcName.left;
	rtBlcTime.right = rtBlcName.right;
	rtBlcTime.top = rtBlcName.bottom + edge.cy;
	rtBlcTime.bottom = rtBlcTime.top + dy;

	//白方分组框
	rtSWht.left = rtSBlc.left;
	rtSWht.right = rtSBlc.right;
	rtSWht.top = rtSBlc.bottom + 1;
	rtSWht.bottom = rtSWht.top + rtSBlc.bottom - rtSBlc.top;

	//白方引擎名静态框
	rtWhtName.left = rtBlcName.left;
	rtWhtName.right = rtBlcName.right;
	rtWhtName.top = rtSWht.top + 2 * edge.cx;
	rtWhtName.bottom = rtWhtName.top + dy;

	//白方计时静态框
	rtWhtTime.left = rtBlcName.left;
	rtWhtTime.right = rtBlcName.right;
	rtWhtTime.top = rtWhtName.bottom + edge.cy;
	rtWhtTime.bottom = rtWhtTime.top + dy;
	...
	if (!SetWindowPos(
		...
		HWND_TOP,
		rtSWht.left + edge.cx,
		rtSWht.bottom + 1 + edge.cy,
		rtSWht.right - rtSWht.left + 1 - 2 * edge.cx,
		rtInfoPart.bottom - rtSWht.bottom - 2 * edge.cy,
		SWP_SHOWWINDOW
		))
		return false;

	dy++;
	dx = (int)((rtBlcName.right - rtBlcName.left + 1 - edge.cx) / 2);
